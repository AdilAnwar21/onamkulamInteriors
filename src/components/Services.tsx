...
const ServicesScroll = memo(({ scrollProgress }: ServicesScrollProps) => {
  ...
  // Math for smooth scrolling
  const progress = Math.max(0, Math.min(1, scrollProgress));
  const adjustedProgress = isActive ? Math.max(0, (progress - threshold) / (1 - threshold)) : 0;
  
  const smoothIndex = adjustedProgress * totalItems;
  const activeIndex = Math.min(Math.floor(smoothIndex), totalItems - 1);
      ...
      <div className="flex flex-col justify-center items-start w-full lg:w-1/2 px-6 sm:px-8 md:px-12 lg:px-16 py-8 lg:py-0 relative z-10">
        ...
        <div className="mb-4 lg:mb-6 min-h-[4rem] flex items-center w-full">
           <h2 
             ...
             style={{ color: '#F0E5DA' }}
           >
             {isActive ? currentItem.title : 'Our Services'}
           </h2>
        </div>
        ...
        >
            ...
            <div 
                ...
                style={{
                    ...
                    transform: `translate3d(0, -${adjustedProgress * (100 - (100/totalItems))}%, 0)`, 
                    willChange: 'transform'
                }}
            >
